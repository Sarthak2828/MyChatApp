...
const Register = () => {
  ...
  e.preventDefault();
  try {
    const result = await login(formData); // Make sure login returns a promise
    if (result?.success) {
      navigate("/");
    } else {
  ...
  return (
    <div className="bg-base-100 text-base-content min-h-screen flex flex-col md:flex-row">
      <Toaster
        toastOptions={{
          duration: 5000,
          style: {
      ...
      {/* Right Section */}
      <div className="w-full md:w-1/2 flex items-center justify-center p-8 mt-3 bg-base-100">
